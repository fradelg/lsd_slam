...
void DepthMap::propagateDepth(Frame* new_keyframe)
{
    ...
    const bool* trackingWasGood = new_keyframe->getTrackingParent() == activeKeyFrame
    ...
    for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++) {
            ...
            if (trackingWasGood != 0) {
                if (!trackingWasGood[(x >> SE3TRACKING_MIN_LEVEL) + (width >> SE3TRACKING_MIN_LEVEL) * (y >> SE3TRACKING_MIN_LEVEL)] || destAbsGrad < MIN_ABS_GRAD_DECREASE) {
                    if (enablePrintDebugInfo)
    ...
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            if (maxGradients[x + y * width] > MIN_ABS_GRAD_CREATE) {
                ...
                currentDepthMap[x + y * width] = DepthMapPixelHypothesis(idepth, idepth, VAR_RANDOM_INIT_INITIAL,
    ...
    std::deque<std::shared_ptr<Frame> > referenceFrames)
{
    ...
    if (enablePrintDebugInfo && printLineStereoFails) {
        printf(
            ...
            runningStats.num_stereo_invalid_unclear_winner + runningStats.num_stereo_invalid_atEnd + runningStats.num_stereo_invalid_inexistantCrossing + runningStats.num_stereo_invalid_noCrossing + runningStats.num_stereo_invalid_twoCrossing + runningStats.num_stereo_invalid_bigErr,
...
void DepthMap::createKeyFrame(Frame* new_keyframe)
{
    ...
    if (plotStereoImages) {
        ...
        cv::cvtColor(debugImageHypothesisPropagation,
                     debugImageHypothesisPropagation, cv::COLOR_GRAY2RGB);
    }
    ...
    RunningStats* stats)
{
    ...
    // the bottom
    if (firstX <= 0 || firstX >= width - 2 || firstY <= 0 || firstY >= height - 2 || lastX <= 0 || lastX >= width - 2 || lastY <= 0 || lastY >= height - 2) {
        return -1;
    }
    ...
    float realVal_p1 = getInterpolatedElement(activeKeyFrameImageData, u + epxn * rescaleFactor,
                                              v + epyn * rescaleFactor, width);
    float realVal_m1 = getInterpolatedElement(activeKeyFrameImageData, u - epxn * rescaleFactor,
                                              v - epyn * rescaleFactor, width);
    ...
    if (pFar[0] <= SAMPLE_POINT_TO_BORDER || pFar[0] >= width - SAMPLE_POINT_TO_BORDER || pFar[1] <= SAMPLE_POINT_TO_BORDER || pFar[1] >= height - SAMPLE_POINT_TO_BORDER) {
        if (enablePrintDebugInfo)
            stats->num_stereo_inf_oob++;
        return -1;
    }
    ...
    if (pClose[0] <= SAMPLE_POINT_TO_BORDER || pClose[0] >= width - SAMPLE_POINT_TO_BORDER || pClose[1] <= SAMPLE_POINT_TO_BORDER || pClose[1] >= height - SAMPLE_POINT_TO_BORDER) {
        ...
        // test again
        if (pClose[0] <= SAMPLE_POINT_TO_BORDER || pClose[0] >= width - SAMPLE_POINT_TO_BORDER || pClose[1] <= SAMPLE_POINT_TO_BORDER || pClose[1] >= height - SAMPLE_POINT_TO_BORDER || newEplLength < 8.0f) {
            if (enablePrintDebugInfo)
