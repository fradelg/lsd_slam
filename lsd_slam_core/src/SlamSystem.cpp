...
bool SlamSystem::updateKeyframe()
{
    ...
    while (unmappedTrackedFrames.size() > 0 && (!unmappedTrackedFrames.front()->hasTrackingParent() || unmappedTrackedFrames.front()->getTrackingParent() != currentKeyFrame.get())) {
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
    ...
    if (!trackingIsGood) {
        ...
        if (displayDepthMap) {
            ...
            cv::cvtColor(tmp_img, tmp_img, cv::COLOR_GRAY2RGB);
    ...
    KFConstraintStruct* e1, KFConstraintStruct* e2)
{
    ...
    if (constraintTracker->diverged || BtoA.scale() > 1 / Sophus::SophusConstants<sophusType>::epsilon() || BtoA.scale() < Sophus::SophusConstants<sophusType>::epsilon() || BtoAInfo(0, 0) == 0 || BtoAInfo(6, 6) == 0) {
        return 1e20;
    }
    ...
    if (constraintTracker->diverged || AtoB.scale() > 1 / Sophus::SophusConstants<sophusType>::epsilon() || AtoB.scale() < Sophus::SophusConstants<sophusType>::epsilon() || AtoBInfo(0, 0) == 0 || AtoBInfo(6, 6) == 0) {
        return 1e20;
    }
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame* newKeyFrame, bool forceParent, bool useFABMAP, float closeCandidatesTH)
{
    ...
    if (parent != 0 && forceParent) {
        ...
        if (e1 != 0) {
            ...
            constraints.back()->information << 0.8098, -0.1507, -0.0557, 0.1211, 0.7657, 0.0120, 0,
