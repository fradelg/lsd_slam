...
#include "lsd_slam_viewer/LsdViewerParamsConfig.h"
#include "settings.h"

PointCloudViewer* viewer = 0;

void dynConfCb(lsd_slam_viewer::LsdViewerParamsConfig& config, uint32_t level)
{
...
void rosThreadLoop(int argc, char** argv)
{
  ...
  dynamic_reconfigure::Server<lsd_slam_viewer::LsdViewerParamsConfig> srv;
...
int main(int argc, char** argv)
{
  ...
  if (argc > 1) {
    rosThread = std::thread(rosFileLoop, argc, argv);
  } else {
    // start ROS thread
    rosThread = std::thread(rosThreadLoop, argc, argv);
  }
